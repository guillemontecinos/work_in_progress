...
function keyPressed(){
  ...
  else if (key === '5' && blackoutScene == true){
    scene = 5;
    blackoutScene = false;
    pantallas.setupPantallas();
  }
...
WebMidi.enable(function (err) {
  ...
  input.addListener('noteon', "all",
    function (e) {
      ...
      else if (e.note.name + parseInt(e.note.octave+2) == "A0") {
        console.log("A0 received");
        scene = 5;
        blackoutScene = false;
        pantallas.setupPantallas();
      }
      ...
      // new pantallas colores trigger
      pantallas.triggerScreen(e.note.name + parseInt(e.note.octave+2));
    }
  );
});
